...
segmentsList.forEach(function(segment) {
  ...
  if (sentence.length) {
    ...
    sentence.forEach(function(word) {
      line = `${line} <span class='transcriptor' data-stime='${parseFloat(word['-stime'])}' data-etime='${parseFloat(word['-stime']) + parseFloat(word['-dur'])}' style='color:${getColor(word['-score'])}'>${word['#text']}</span>`;
    });
    ...
    line = `<span class='transcriptor' data-stime='${parseFloat(sentence['-stime'])}' data-etime='${parseFloat(sentence['-stime']) + parseFloat(sentence['-dur'])}' style='color:${getColor(sentence['-score'])}'>${sentence['#text']}</span>`
    o = {begin: segment['-stime'], children: [], end: String((parseFloat(segment['-stime'])+ parseFloat(segment['-dur']))), id: String(notes.length), language: 'eng', lines: [line] };
    notes.push(o);
  }
