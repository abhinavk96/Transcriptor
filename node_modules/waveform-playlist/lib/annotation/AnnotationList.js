    ...
    value: function render() {
        ...
        return (0, _h2.default)('div.annotation-box', {
          ...
          onclick: function onclick() {
            if (_this4.playlist.isContinuousPlay) {
              ...
              console.log('here');
              _this4.playlist.ee.emit('select', _this4.annotations[i].start, _this4.annotations[i].end);
      ...
      var text = (0, _h2.default)('div.annotations-text', {
        ...
        if (_this4.playlist.isPlaying() && _this4.playlist.playbackSeconds >= note.start && _this4.playlist.playbackSeconds <= note.end) {
          segmentClass = '.current.ui.blue.message';
          // console.log('a segment is playing', note.lines);
          // var parser = new DOMParser();
          // var htmlDoc = parser.parseFromString(note.lines, 'text/html');
          // var spans = $(htmlDoc.getElementsByClassName('transcriptor'));
          // spans.each(index => {
          //   $(spans[index]).addClass('currentWord');
          // })
          // console.log(String(spans[0].outerHTML));
          // // note.lines = `${spans[0].outerHTML}`;
          // note.lines = [String(`${spans[0].outerHTML}`)];
